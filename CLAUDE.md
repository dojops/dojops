...
- `ScopeSchema` / `RiskSchema` / `ExecutionSchema` / `UpdateSchema` (`packages/runtime/src/spec.ts`) — Zod schemas for `.dops` frontmatter sections: write boundary enforcement, risk self-classification (LOW/MEDIUM/HIGH), execution semantics (mode/deterministic/idempotent), and structured update behavior (strategy/inputSource/injectAs). All optional for backward compat
- `DopsRuntime.risk` / `.executionMode` / `.isDeterministic` / `.isIdempotent` (`packages/runtime/src/runtime.ts`) — getters exposing `.dops` frontmatter data with safe defaults (risk defaults to LOW, execution defaults to generate/non-deterministic/non-idempotent)
- `ToolMetadata.riskLevel` / `.icon` (`packages/runtime/src/runtime.ts`) — risk classification and optional marketplace icon URL (HTTPS only, max 2048 chars) exposed in tool metadata
- `matchesScopePattern()` (`packages/runtime/src/file-writer.ts`) — scope write boundary enforcement: validates resolved file paths against declared `scope.write` patterns after `{var}` expansion
